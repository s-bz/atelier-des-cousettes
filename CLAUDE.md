...
- `robots.txt` explicitly allows AI crawlers (GPTBot, ChatGPT-User, ClaudeBot, Claude-SearchBot, PerplexityBot, Perplexity-User). Anthropic's crawler is **ClaudeBot** (and **Claude-SearchBot** for search) — `Claude-Web` is a retired name that no longer designates anything, and `robots.txt` says so in a comment. Check that file before copying a user-agent name from memory.
