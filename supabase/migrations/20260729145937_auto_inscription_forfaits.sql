-- L'auto-inscription étale un forfait au lieu de le brûler.
--
-- Un forfait est acquis en totalité dès le premier jour — c'est ce qui permet
-- d'en placer plusieurs le même mois. Mais l'auto-inscription ne doit pas s'en
-- servir pour tout réserver d'avance : dix séances posées sur les dix premiers
-- jeudis épuiseraient la saison avant Noël, et quelqu'un qui ne visite jamais
-- le site se retrouverait sans rien au printemps.
--
-- On répartit donc sur les mois de l'abonnement. La cible n'est pas un taux
-- mensuel arrondi mais un CUMUL : au mois k, on veut round(k × total / mois)
-- séances posées. Les restes se placent d'eux-mêmes — quinze séances sur dix
-- mois donnent 2,1,2,1,2,1,2,1,2,1 sans qu'aucun arrondi ne se perde.
--
-- Un forfait mensuel garde exactement le comportement précédent.
